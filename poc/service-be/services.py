...
async def initialize_mongodb_client() -> AsyncIOMotorClient:
    try:
        ...
        raise HTTPException(status_code=500, detail="Timeout while connecting to MongoDB")
    except Exception as e:
        logger.error(f"Failed to initialize MongoDB client: {e}")
        raise HTTPException(status_code=500, detail=f"Failed to initialize MongoDB client: {str(e)}")
